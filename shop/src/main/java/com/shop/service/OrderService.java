...
public class OrderService {
    ...
    public Page<OrderHistDto> getOrderList(String email, Pageable pageable) {
        List<Order> orders = orderRepository.findOrders(email, pageable); // 유저의 아이디와 페이징 조건을 이용하여 주문 목록을 조회한다.
        Long totalCount = orderRepository.countOrder(email); // 유저의 주문 총 개수를 구한다.

        List<OrderHistDto> orderHistDtos = new ArrayList<>();

        for (Order order : orders) { // 주문 리스트를 반복하면서 구매 이력 페이지에 전달할 DTO를 생성한다.
    ...
    @Transactional(readOnly = true)
    public boolean validateOrder(Long orderId, String email) { // 현재 로그인 한 사용자와 주문 데이터를 생성한 사용자가 같은 지를 검사한다.
        Member curMember = memberRepository.findByEmail(email);
        Order order = orderRepository.findById(orderId)
                .orElseThrow(EntityNotFoundException::new);
        Member savedMember = order.getMember();

        if (!StringUtils.equals(curMember.getEmail(), savedMember.getEmail())) { // 같지 않을 경우에는 false를 반환한다.
            return false;
        }

        return true; // 같을 때는 true를 반환한다.
    }
    ...
    public Long orders(List<OrderDto> orderDtoList, String email) {
        ...
        for (OrderDto orderDto : orderDtoList) { // 주문할 상품 리스를 만든다.
