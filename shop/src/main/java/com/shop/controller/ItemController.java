...
public class ItemController {
    ...
    @GetMapping(value = {"/admin/items", "/admin/items/{page}"}) // value에 상품 관리 화면 진입 시 URL에 페이지 번호가 없는 경우와 페이지 번호가 있는 경우 2가지를 매핑한다.
    public String itemManage(ItemSearchDto itemSearchDto, @PathVariable("page") Optional<Integer> page, Model model) {
        Pageable pageable = PageRequest.of(page.isPresent() ? page.get() : 0, 3); // 페이징을 위해서 PageRequest.of() 메서드를 통해 Pageable 객체를 생성한다. 첫 번째 파라미터로는 조회할 페이지 번호, 두 번째 파라미터로는 올 데이터 수를 넣어준다. URL 경로에 페이지 번호가 있으면 해당 페이지를 조회하도록 설정하고, 페이지 번호가 없으면 0페이지를 조회하도록 한다.
        ...
        model.addAttribute("itemSearchDto", itemSearchDto); // 페이지 전환 식 ㅣ존 검색 조건을 유지한 채 이동할 수 있도록 뷰에 다시 전달한다.
