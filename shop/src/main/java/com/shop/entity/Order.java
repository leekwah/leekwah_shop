...
public class Order {
    ...
    @OneToMany(mappedBy = "order", cascade = CascadeType.ALL) // 주문 상품 엔티티와 일대다 매핑을 한다. 외래키(order_id)가 order_item 테이블에 있으므로 연관 관계의 주인은 OrderItem 엔티티이다. Order 엔티티가 주인이 아니므로 mappedBy 속성으로 연관 관계의 주인을 설정한다. 연관 관계의 주인의 필드인 order를 mappedBy의 값으로 세팅하면 된다. CascadeType.All 옵션은 부모 엔티티의 영속성 상태 변화를 자식 엔티티에 모두 전이하는 것이다.
