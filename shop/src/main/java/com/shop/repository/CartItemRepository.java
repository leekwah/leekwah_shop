...
public interface CartItemRepository extends JpaRepository<CartItem, Long> {
     ...
     */
    @Query("SELECT new com.shop.dto.CartDetailDto(ci.id, i.itemNm, i.price, ci.count, im.imgUrl) " + // CartDetailDto의 생성자를 이용하여 DTO를 반환할 때는 new 키워드와 해당 DTO의 패키지, 클래스명을 함께 적어준다. 또한 생성자의 파라미터 순서는 DTO 클래스에 명시한 순서대로 넣어줘야한다.
            "FROM CartItem ci, ItemImg im " +
            "JOIN ci.item i " +
            "WHERE ci.cart.id = :cartId " +
            "AND im.item.id = ci.item.id " +
            "AND im.repImgYn = 'Y' " + // 장바구니에 담겨있는 상품의 대표 이미지만 가져오도록 조건문을 작성한다.
